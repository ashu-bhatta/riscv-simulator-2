...
int main(int argc, char *argv[]) {
  ...
  std::unique_ptr<VmBase> vm = createVm(); // <-- Use the factory to create the initial VM
  ...
  while (true) {
    ...
    if (command.type==command_handler::CommandType::MODIFY_CONFIG) {
      ...
      try {
        ...
        // If processor type changed, recreate the VM
        if (command.args[0] == "Execution" && command.args[1] == "processor_type") {
            vm = createVm();
            // If a program was already loaded, reload it into the new VM
            if (program.text_buffer.size() > 0) {
                vm->LoadProgram(program);
            }
        }
    ...
    if (command.type==command_handler::CommandType::LOAD) {
      try {
        ...
        // Recreate the VM to ensure it's clean, and load the program
        vm = createVm();
